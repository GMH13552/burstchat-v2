...
def preprocess(messages: list[Message], target: str, max_chars: int = 24000) -> dict:
    ...
    # ── 高频词统计（只统计含中文的 n-gram）──
    from collections import Counter
    import unicodedata
    def is_mostly_chinese(s):
        """至少 60% 是中文字符"""
        chinese = sum(1 for ch in s if '\u4e00' <= ch <= '\u9fff' or '\u3400' <= ch <= '\u4dbf')
        total = len(s)
        return total > 0 and chinese / total >= 0.6
    ...
    for m in their_msgs:
        ...
        for i in range(len(c) - 1):
            for L in [2, 3, 4]:
                if i + L <= len(c):
                    w = c[i:i+L]
                    if is_mostly_chinese(w):
                        word_counter[w] += 1
    word_counter = Counter({k: v for k, v in word_counter.items() if v >= 3})
    top_words = {w for w, _ in word_counter.most_common(40)}
    top_bigrams = {w for w, _ in word_counter.most_common(20) if len(w) >= 3}
    all_priority_words = top_words | top_bigrams
    top_display = [f"{w}({word_counter[w]})" for w, _ in word_counter.most_common(15)]
    print(f"  高频词: {', '.join(top_display)}")
    ...
    for m in their_msgs:
        ...
        if any(kw in c for kw in conflict_kw):
            ...
            # 检查是否含高频词（子串匹配）
            has_priority = any(w in c for w in all_priority_words)
            if has_priority:
                high_freq_msgs.append(m)
            elif len(c) > 50:
                long_msgs.append(m)
            else:
                daily_msgs.append(m)
    ...
    # 高频词消息(识别人物口癖和表达风格的关键)
    added, remaining = add_batch(high_freq_msgs, "high_freq", min(remaining, 8000))
    lines += added

    added, remaining = add_batch(sweet_msgs, "sweet", min(remaining, 3000))
    lines += added
    ...
    # ── 统计摘要(传入高频词数据)──
    stats = compute_stats(their_msgs, my_msgs, messages, word_counter)
...
def compute_stats(their_msgs: list[Message], my_msgs: list[Message],
                  all_msgs: list[Message], word_counter=None) -> dict:
    """基础统计 + 高频词(给 LLM 做参考)"""
    ...
    stats = {
        ...
        "top_emojis": [f"{e}({n}次)" for e, n in emoji_counter.most_common(10)],
    ...
    # 高频词
    if word_counter:
        stats["top_phrases"] = [f"{w}({n}次)" for w, n in word_counter.most_common(20) if len(w) >= 2]
    return stats
...
**身份与年龄**(必须从聊天内容中精确推断,不要模糊表述。例如聊到"我校""学院""双学位""大二""考研"等就写具体学校/年级,不要写"20-25岁学生")

**口头禅与高频词**(从下方的统计数据和采样消息中提取,列出实际出现频率最高的10个,每个附使用场景和出现次数)

**句式特征**(短句流/小作文/混合、是否用句号、感叹号/省略号习惯)

**emoji/表情使用**(从下方的统计数据中取实际频率最高的,不要自己猜!列5个最常用的,附使用次数和场景)
...
2. 不要写形容词,要写具体行为。
3. 身份/年龄必须精确--如果聊天中提到学校、专业、年级、工作等,必须提取。
4. emoji 必须来自统计数据中的 top_emojis,不要自己编造。
5. 口头禅必须来自统计数据中的 top_phrases,不要编造不存在的词。
6. 如果某个维度信息不足,标注 `(聊天记录不足,以下为推断)`。
7. 使用中文输出。"""
